...
/**
 ...
 There are several static helper methods, which allow presenting a basic snackywithout needing instantiate an `ACSnacky` yourself.
 */
open class ACSnacky: Equatable {
    ...
    /**
     Whether or not the ACSnacky should adjust to fit within the safe area's of it's parent view.
    ...
    /// The completion block for an `ACSnacky`, `true` is sent if button was tapped, `false` otherwise.
    ...
    internal func frameForView() -> CGRect {
        ...
        // Check to see if a ACSnacky is already being presented in this view
    ...
    // MARK: Static Methods
    /// Allows showing a simple snackywithout needing to instantiate any `ACSnacky`
    public static func show(view: UIView? = nil, title: String, displayDuration: TimeInterval? = 5.0, completion: SnackyCompletion? = nil) {
        let snacky = ACSnacky(title: title, buttonTitle: nil)
    ...
    /// Allows showing a simple, more customizable, snackywithout needing to instantiate any `ACSnacky`
